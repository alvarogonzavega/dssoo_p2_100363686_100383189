	...
	if(blockI == blockF){
		...
		for(int i=blockI; i<blockF; i++){
			...
			if(i==blockF-1){
				...
				bwrite(disk, inodo[fileDescriptor].block[i], wbf);
				total += BLOCK_SIZE;
				inodo[fileDescriptor].size += BLOCK_SIZE; //We update the size of the file
...
int removeLn(char *linkName)
{
	...
	for(int j=0; j<i; j++){
		...
		while((amp[k] = strtok_r(tok[j], ampersand, &tok[j]))){
			...
			if(strcmp(amp[k], linkName)==0) break;
			k++;
	...
	for(int j=0; j<i; j++){
		...
		if(j!=i){
			...
			strncat(new, "|", 1);
			strncat(new, tok[j], strlen(tok[j]));
	...
	lseekFile(of_result, 0, FS_SEEK_BEGIN);
	if (writeFile(of_result, links_buffer, link_length) == -1) return -2;
